...
namespace Sungero.Projects.Server
{
  public class ModuleFunctions
  {
    ...
    public static List<Structures.ProjectDocumentRightsQueueItem.ProxyQueueItem> CreateAccessRightsProjectDocumentQueueItemWithAddendum(int documentId, int projectId)
    {
      ...
      var addendums = GetAddendums(document);
    ...
    {
      var addemdumsList = new List<IOfficialDocument>() { };
      var leadindDocuments = new List<IOfficialDocument>() { document };
      while (Addendums.GetAll(a => leadindDocuments.Contains(a.LeadingDocument)).Any())
      {
        leadindDocuments = OfficialDocuments.GetAll(a => Addendums.Is(a) && leadindDocuments.Contains(a.LeadingDocument)).ToList();
        addemdumsList.AddRange(leadindDocuments);
      }
      
      return addemdumsList;
    }
    ...
    {
      var grantRightDocument = document;
      while (document.LeadingDocument != null)
      {
        ...
        this.AddRecipientRightsForProject(grantRightDocument, leadingProject, document.LeadingDocument.DocumentKind.GrantRightsToProject.Value, result);
    ...
    /// <summary>
    /// Получить список реципиентов для вдачи прав.
    /// </summary>
    /// <param name="project">Проект.</param>
