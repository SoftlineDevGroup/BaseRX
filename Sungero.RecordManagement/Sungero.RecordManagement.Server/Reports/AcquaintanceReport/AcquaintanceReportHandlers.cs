  ...
  partial class AcquaintanceReportServerHandlers
  {
    ...
    public override void BeforeExecute(Sungero.Reporting.Server.BeforeExecuteEventArgs e)
    {
      ...
      if (validationMessages.Any())
      {
        ...
        AcquaintanceReport.SignValidationErrors = string.Join(Environment.NewLine, validationMessages);
      }
      ...
      // Приложения.
      var documentAddendas = Functions.Module.GetAcquintanceTaskAddendas(tasks);
      if (documentAddendas.Any())
      {
        AcquaintanceReport.AddendaDescription = Reports.Resources.AcquaintanceReport.Addendas;
        foreach (var addenda in documentAddendas)
        {
          var addendaInfo = string.Format("\n - {0} ({1}:{2}{3}).", addenda.DisplayValue.Trim(),
                                          Docflow.Resources.Id, nonBreakingSpace, addenda.Id);
          AcquaintanceReport.AddendaDescription += addendaInfo;
        }
      }
