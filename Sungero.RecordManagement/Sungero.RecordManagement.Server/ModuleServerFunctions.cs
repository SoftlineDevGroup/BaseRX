  ...
  public class ModuleFunctions
  {
    ...
    public static IActionItemExecutionTask CreateActionItemExecution(IOfficialDocument document, IAssignment parentAssignment)
    {
      ...
      if (parentAssignment != null)
      {
        ...
        var addendas = task.AddendaGroup.All;
        foreach (var otherGroupAttachment in parentAssignment.AllAttachments.Where(x => !Equals(x, document) && !addendas.Contains(x) && !resolutions.Contains(x)))
    ...
    /// <summary>
    /// Добавить фильтры по статусу, замещению, периоду.
    /// </summary>
    /// <param name="query">Запрос.</param>
    /// <param name="inProcess">Признак показа заданий "В работе".</param>
    /// <param name="shortPeriod">Фильтр по короткому периоду.</param>
    /// <param name="middlePeriod">Фильтр по среднему периоду.</param>
    /// <param name="longPeriod">Фильтр по длинному периоду.</param>
    /// <param name="longPeriodToCompleted">Фильтр по длинному периоду для завершённых заданий.</param>
    ...
    /// <summary>
    /// Добавить фильтры по статусу, периоду для задач.
    /// </summary>
    /// <param name="query">Запрос.</param>
    /// <param name="inProcess">Признак показа заданий "В работе".</param>
    /// <param name="shortPeriod">Фильтр по короткому периоду.</param>
    /// <param name="middlePeriod">Фильтр по среднему периоду.</param>
    /// <param name="longPeriod">Фильтр по длинному периоду.</param>
    /// <param name="longPeriodToCompleted">Фильтр по длинному периоду для завершённых задач.</param>
    /// <returns>Отфильтрованный запрос.</returns>
    [Public]
    public IQueryable<Sungero.Workflow.ITask> ApplyCommonSubfolderFilters(IQueryable<Sungero.Workflow.ITask> query,
                                                                          bool inProcess,
    ...
    {
      var addendas = new List<IElectronicDocument>();
      var addendasIds = tasks.SelectMany(x => x.AcquaintanceVersions)
        .Where(x => x.IsMainDocument != true)
        .Select(x => x.DocumentId);
      
      var documentAddendas = tasks.SelectMany(x => x.AddendaGroup.OfficialDocuments)
        .Where(x => addendasIds.Contains(x.Id))
        .Distinct()
        .ToList();
      addendas.AddRange(documentAddendas);
