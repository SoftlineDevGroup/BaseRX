...
namespace Sungero.Docflow.Client
{
  partial class OfficialDocumentFunctions
  {
    ...
    /// <summary>
    /// Проверить ифомацию о блокировках тела и карточки документа.
    /// </summary>
    /// <param name="document">Документ.</param>
    /// <returns>Информация о блокировке.</returns>
    ...
    /// <summary>
    /// Выбор связанных документов для отправки и создание письма.
    /// </summary>
    ...
    public virtual void CreateActionItemsFromDocumentDialog(Sungero.Core.IValidationArgs e)
    {
      ...
      Action<CommonLibrary.InputDialogRefreshEventArgs> refresh = _ =>
      {
        ...
        else
        {
          ...
          var isAllSended = dialogItems.All(d => d.Status != RecordManagement.ActionItemExecutionTask.Status.Draft);
          next.IsVisible = !isAllSended;
    ...
    public static bool NeedToAttachDocument(Content.IElectronicDocument attachment, Docflow.IOfficialDocument mainOfficialDocument)
    {
      ...
      var addendums = mainOfficialDocument.Relations.GetRelated(Constants.Module.AddendumRelationName);
      if (addendums.Any(ad => Equals(ad.Id, attachment.Id)))
        return false;
      return true;
    }
