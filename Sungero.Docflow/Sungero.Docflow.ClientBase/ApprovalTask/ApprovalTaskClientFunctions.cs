  ...
  partial class ApprovalTaskFunctions
  {
    ...
    public bool GetReasonBeforeAbort(string activeText)
    {
      ...
      if (document != null)
      {
        ...
        isDocumentClosed = dialog.AddBoolean(textToMarkDocumentAsObsolete, true);
      ...
      if (dialog.Show() == DialogButtons.Ok)
      {
        ...
        {
          Sungero.Docflow.PublicFunctions.OfficialDocument.SetObsolete(document, true);
          document.Save();
        }
        return true;
      }
    ...
    /// <param name="info">Информация о свойстве.</param>
    /// <param name="deliveryMethod">Сервис обмена.</param>
    /// <param name="param">Параметр, в котором хранится иформация необходимости показать хинт.</param>
    /// <returns>Признак необходимости показать хинт. True - если нужно показать хинт, инче - false.</returns>
