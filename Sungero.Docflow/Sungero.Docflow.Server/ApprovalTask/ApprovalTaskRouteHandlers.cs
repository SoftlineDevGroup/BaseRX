...
namespace Sungero.Docflow.Server
{
  partial class ApprovalTaskRouteHandlers
  {
    ...
    public virtual void StartBlock42(Sungero.Docflow.Server.ApprovalNotificationArguments e)
    {
      ...
      if (ApprovalSigningAssignments.Is(lastAssigment) && lastAssigment.Result == Docflow.ApprovalSigningAssignment.Result.Abort ||
          ApprovalReviewAssignments.Is(lastAssigment) && lastAssigment.Result == Docflow.ApprovalReviewAssignment.Result.Abort)
    ...
    public virtual void StartBlock3(Sungero.Docflow.Server.ApprovalManagerAssignmentArguments e)
    {
                              ...
                              (!isReworkPerformer && reworkAssignment != null)))
      {
        ...
        #region Сформировать список согласюущих
    ...
    public virtual bool Monitoring29Result()
    {
      ...
      // Если есть выдача от текущей задачи или выдачи, созданные ручной отправка документа КА через сервис обмена.
    ...
    public virtual void CompleteAssignment27(Sungero.Docflow.IApprovalCheckReturnAssignment assignment, Sungero.Docflow.Server.ApprovalCheckReturnAssignmentArguments e)
    {
      ...
      {
        // Получить всех кто учавствовал в согласовании и подписании.
    ...
    {
      var assigment = ApprovalCheckReturnAssignments.GetAll(a => Equals(a.Task, _obj) && a.Created >= _obj.Started && a.Result != null)
        .OrderByDescending(a => a.Completed)
        .FirstOrDefault();
      return assigment.Result == Docflow.ApprovalCheckReturnAssignment.Result.Signed;
    }
