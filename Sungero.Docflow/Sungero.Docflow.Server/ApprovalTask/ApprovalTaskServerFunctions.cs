  ...
  partial class ApprovalTaskFunctions
  {
                                                 ...
                                                 bool hasAbort, bool hasTaskAbort, StateBlockIconSize iconSize)
    {
      ...
      if (nextAssignment != null && !nextAssignment.Result.HasValue &&
          nextAssignment.BlockId == Constants.Module.ApprovalReworkAssignmentBlockId)
      {
        block.AssignIcon(StateBlockIconType.Abort, iconSize);
    ...
    public static void IssueDocument(IApprovalTask task, int returnResponcibleID)
    {
      ...
      if (tracking != null)
      {
        ...
        foreach (var addenda in task.AddendaGroup.OfficialDocuments.Where(x => x.Tracking.Any(t => t.ReturnResult == null && t.ReturnTask == null && t.ExternalLinkId != null)).ToList())
        {
          var addendaTracking = addenda.Tracking.LastOrDefault(x => x.ReturnResult == null && x.ReturnTask == null && x.ExternalLinkId != null && x.Action == Docflow.OfficialDocumentTracking.Action.Endorsement);
          
          if (addendaTracking != null)
          {
            addendaTracking.ReturnTask = task;
            
            if (returnStage != null)
              addendaTracking.DeliveredTo = tracking.DeliveredTo;
          }
        }
      }
    ...
    public virtual void ApprovalRuleChanged(IApprovalRuleBase rule, List<Structures.Module.DefinedApprovalStageLite> stages)
    {
      ...
      if (rule != null)
      {
        ...
        {
          // Заполнить подписывающего из документа, если он разрешен для правила.
          if (document != null)
          {
            ...
            {
              if (allowedSignatories.Any(s => Equals(s.EmployeeId, ourSignatory.Id)))
                _obj.Signatory = ourSignatory;
            }
            else
              _obj.Signatory = Docflow.Functions.OfficialDocument.GetDefaultSignatory(document, allowedSignatories);
          }
        }
        ...
        if (reviewStage != null)
        {
          ...
          if (reviewStage.Stage.AssigneeType == Sungero.Docflow.ApprovalStage.AssigneeType.Employee &&
                                  ...
                                  bool isRequiredApprovers)
    {
      ...
      foreach (var approver in approvers)
      {
        ...
        // Новое или повторное согласование (плюс переадесации -- когда в гриде было без отправки, а задание всё таки есть).
    ...
    public List<IUser> GetAllApproversAndSignatories()
    {
      ...
      return Assignments.GetAll()
        ...
        .Where(a => approvalBlocks.Contains(a.BlockId ?? 0))
    ...
    /// <summary>
    /// Обновить доп. согласующих  задаче.
    /// </summary>
        ...
        if (assignmentsInProcess.Any())
        {
              ...
              stage.ApprovalRole == null)
          {
            ...
            var employeeWithAssignments = assignments
              .Where(a => a.BlockId == assignment.BlockId &&
    ...
    public static Structures.ApprovalTask.RefreshParameters GetStagesInfoForRefresh(IApprovalTask task, List<Structures.Module.DefinedApprovalStageLite> stages)
    {
      ...
      info.AddresseeIsVisible = hasReviewStage;
      info.SignatoryIsVisible = hasSignStage;
    ...
    public virtual IUser GetLastAssignmentPerformer()
    {
      ...
      var lastAssigment = Functions.ApprovalTask.GetLastTaskAssigment(_obj, null);
      
      // Если это подписание, то инициатор - подписывающий.
      if (ApprovalSigningAssignments.Is(lastAssigment))
      {
        var signAssigment = ApprovalSigningAssignments.As(lastAssigment);
        performer = signAssigment.Performer;
      }
      // Если это рассмотрение, то инициатор - адресат.
      if (ApprovalReviewAssignments.Is(lastAssigment))
      {
        var reviewAssigment = ApprovalReviewAssignments.As(lastAssigment);
        performer = reviewAssigment.Performer;
      }
      if (ApprovalCheckReturnAssignments.Is(lastAssigment))
      {
        var checkAssigment = ApprovalCheckReturnAssignments.GetAll()
        ...
        performer = checkAssigment.Performer;
      }
      if (ApprovalAssignments.Is(lastAssigment))
      {
        var apprAssigment = ApprovalAssignments.GetAll()
          .Where(ass => Equals(ass.Task, _obj) && ass.Result == Docflow.ApprovalAssignment.Result.ForRevision)
          .OrderByDescending(o => o.Completed).FirstOrDefault();
        performer = apprAssigment.Performer;
      }
      if (ApprovalManagerAssignments.Is(lastAssigment))
      {
        var apprAssigment = ApprovalManagerAssignments.GetAll()
