...
namespace Sungero.Docflow.Server
{
  partial class DeadlineExtensionTaskRouteHandlers
  {
    ...
    public virtual void Script5Execute()
    {
      ...
      if (RecordManagement.ActionItemExecutionAssignments.As(_obj.ParentAssignment) != null)
      {
        ...
        foreach (var performer in actionItemExecution.CoAssignees)
        {
          ...
          foreach (var subTask in subTasks)
          {
            ...
            var assigment = RecordManagement.ActionItemExecutionAssignments.GetAll()
              .FirstOrDefault(a => Equals(a.Task, subTask) && a.Status == Sungero.Workflow.AssignmentBase.Status.InProcess);
            
            if (assigment != null)
            {
              assigment.Deadline = desiredDeadline;
              assigment.ScheduledDate = desiredDeadline;
            }
          }
        }
      }
      ...
      var parentAssigment = _obj.ParentAssignment;
      if (ApprovalTasks.Is(parentAssigment.MainTask))
      {
        var approvalTask = ApprovalTasks.As(parentAssigment.MainTask);
        approvalTask.MaxDeadline = Functions.ApprovalTask.GetExpectedDate(approvalTask);
      }
      else if (RecordManagement.AcquaintanceTasks.Is(parentAssigment.MainTask))
      {
        var acquaintanceTask = RecordManagement.AcquaintanceTasks.As(parentAssigment.MainTask);
