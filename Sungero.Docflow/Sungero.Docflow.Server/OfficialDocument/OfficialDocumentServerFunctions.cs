...
namespace Sungero.Docflow.Server
{
  partial class OfficialDocumentFunctions
  {
    ...
    /// </summary>
    /// <returns>Признак изменености.</returns>
    ...
    /// </summary>
    /// <returns>Задачи на ознаколмение с документом.</returns>
    ...
    public virtual void CreatePdfPublicBodyFromHtml(string sourceHtml)
    {
      ...
      // Заполнение расширения обязательно. Делать это нужно после создания public-body, иначе затрется оригинальное расширение.
        ...
        using (var stream = new System.IO.MemoryStream())
        {
          ...
          for (int i = 0; i < tagTexts[OfficialDocuments.Resources.ActionItemCreationDialogActionItemTag].Length; i++)
          {
            ...
            else
            {
              ...
              // TODO баг 64473 - нет прав у работника, создавшего поручения, если поручения выдается от имени руководителя.
    ...
    public virtual Structures.OfficialDocument.СonversionToPdfResult ConvertToPdfWithSignatureMark()
    {
      ...
      // Определить способ преобразования: интерактивно/асинхронно.
      if (this.CanConvertToPdfInteractively())
      {
    ...
    public virtual Structures.OfficialDocument.СonversionToPdfResult ValidateDocumentBeforeConvertion(int versionId)
    {
      ...
      // Документ МКДО.
      if (Exchange.ExchangeDocumentInfos.GetAll().Any(x => Equals(x.Document, _obj)))
      {
        info.ErrorTitle = OfficialDocuments.Resources.IsExchangeServiceDocumentTitle;
        info.ErrorMessage = OfficialDocuments.Resources.IsExchangeServiceDocument;
        return info;
      }
