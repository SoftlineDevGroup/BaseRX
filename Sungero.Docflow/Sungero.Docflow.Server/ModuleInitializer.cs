...
namespace Sungero.Docflow.Server
{
  public partial class ModuleInitializer
  {
    ...
    #region Создание типов прав модуля. Измненеие системных прав
    ...
    public static void CreateFileTypesAndAssociatedApps()
    {
      ...
      // Dmitriev_IA: Создание приложения-обработчика Unknown applocation.
    ...
    /// <summary>
    /// Создать журнал для с разрезом нумерации по году.
    /// </summary>
    /// <param name="name">Название.</param>
    ...
    {
      var externlLink = Docflow.PublicFunctions.Module.GetExternalLink(DocumentRegister.ClassTypeGuid, entityId);
      if (externlLink != null)
        return DocumentRegisters.Null;
      
      var exists = DocumentRegisters.GetAll(l => l.Name == name).FirstOrDefault();
      if (exists != null)
      {
        Docflow.PublicFunctions.Module.CreateExternalLink(exists, entityId);
        return exists;
      }
    ...
    public static void CreateCurrency(string name, string alphaCode, string numericCode, string shortName, string fractionName, bool isDefault)
    {
      ...
      var exist = Currencies.GetAll(c => c.NumericCode == numericCode).FirstOrDefault();
      if (exist != null)
      {
        if (string.IsNullOrWhiteSpace(exist.ShortName))
          exist.ShortName = shortName;
        if (string.IsNullOrWhiteSpace(exist.FractionName))
          exist.FractionName = fractionName;
        exist.Save();
        return;
      }
