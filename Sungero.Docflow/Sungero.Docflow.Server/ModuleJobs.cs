...
namespace Sungero.Docflow.Server
{
  public class ModuleJobs
  {
    ...
    public virtual void SendNotificationForExpiringPowerOfAttorney()
    {
      ...
      var alreadySendedDocs = PublicFunctions.Module.GetDocumentsWithSendedTask(notifyParams.ExpiringDocTableName);
      
      var powerOfAttorneyIds = PowerOfAttorneys.GetAll()
        .Where(p => !alreadySendedDocs.Contains(p.Id))
    ...
    /// <param name="allRules">Действущие правила выдачи прав.</param>
    ...
    {
      Logger.DebugFormat("CreateAccessRightsQueueItem: document {0}, rule {1}, rigthType {2}", documentId, rule.Id, rightType);
