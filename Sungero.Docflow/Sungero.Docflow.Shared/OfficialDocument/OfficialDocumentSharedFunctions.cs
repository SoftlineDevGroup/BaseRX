...
namespace Sungero.Docflow.Shared
{
  partial class OfficialDocumentFunctions
  {
    ...
    public static void FillCaseFileAndDeliveryMethod(IOfficialDocument document, IDocumentRegister documentRegister)
    {
      ...
      {
        var adressees = outgoingDocument.Addressees.Where(a => a.DeliveryMethod == null);
        foreach (var adressee in adressees)
          adressee.DeliveryMethod = deliveryMethod;
      }
    ...
    public virtual void ChangeDocumentPropertiesAccess(bool isEnabled, bool repeatRegister)
    {
      ...
      else
      {
        ...
        // При перерегистрации НОР недоступно, если в журнале есть разрез по НОР или в формате номера журнала есть код НОР.
    ...
    public virtual Sungero.Company.IEmployee GetDefaultSignatory(List<Docflow.Structures.SignatureSetting.Signatory> signatories)
    {
      ...
      var signatoriesMaxPriorityCount = signatoriesMaxPriority.Count();
      if (signatoriesMaxPriorityCount == 1)
      {
    ...
    /// <summary>
    /// Определить необходимость защиты от резактирования ведущий документ.
    /// </summary>
