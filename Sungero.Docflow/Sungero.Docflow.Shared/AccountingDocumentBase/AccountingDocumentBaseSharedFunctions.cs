...
namespace Sungero.Docflow.Shared
{
  partial class AccountingDocumentBaseFunctions
  {
    ...
    public static Sungero.FinancialArchive.IFinArchiveExportReport GetFinArchiveExportReport(List<Structures.AccountingDocumentBase.ExportedDocument> objs, DateTime dateTimeNow)
    {
      if (objs.Any())
      {
        var faultedDocuments = objs.Where(d => d.IsFaulted).Select(d => d.Id);
        var addendumsFaulted = objs.Where(d => !d.IsFaulted && d.IsAddendum && d.LeadDocumentId != null && faultedDocuments.Contains(d.LeadDocumentId.Value));
      ...
      report.ReportSessionId = Functions.AccountingDocumentBase.Remote.GenerateFinArchiveExportReport(objs, ".");
