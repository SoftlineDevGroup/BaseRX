...
namespace Sungero.Exchange.Server
{
  partial class ExchangeDocumentInfoFunctions
  {
    ...
    public static bool LastVersionSended(Docflow.IOfficialDocument document)
    {
      ...
      var documentSended = new Enumeration(Constants.Module.Exchange.SendDocument);
      var answerSended = new Enumeration(Constants.Module.Exchange.SendAnswer);
      ...
      if (Docflow.AccountingDocumentBases.Is(document) && Docflow.AccountingDocumentBases.As(document).IsFormalized == true)
        return document.History.GetAll().Any(h => (h.Operation == documentSended || h.Operation == answerSended));
      return document.History.GetAll().Any(h => (h.Operation == documentSended || h.Operation == answerSended) && h.VersionNumber == versionNumber);
    }
    ...
    /// </summary>
    /// <param name="document">Документ.</param>
    /// <returns>Контрагент, от котого пришел документ, либо контрагенты из карточки документа.</returns>
    [Public]
