  ...
  public class ModuleFunctions
  {
                                                   ...
                                                   bool bulkMode)
    {
      ...
      if (!bulkMode &&
          ...
          PublicFunctions.Module.Remote.IsJobEnabled(Constants.Module.CreateReceiptNotifications))
        return string.Empty;
      ...
      while (documentInfos.Any())
      {
        ...
        try
        {
          ...
          if (documentsToFix.Any())
            Functions.Module.Remote.FixReceiptNotification(documentsToFix, string.Empty, false);
    ...
    [Public]
    public static void SendResultToCounterparty(Docflow.IOfficialDocument document, ExchangeCore.IExchangeService service, List<Docflow.IOfficialDocument> addenda)
    {
      ...
      dialog.SetOnButtonClick(x =>
                              {
                                if (x.Button == sendButton && x.IsValid)
                                {
                                  ...
                                  foreach (var addendaDocument in selectedAddenda.Value)
                                    if (IsLocked(addendaDocument, x))
                                      return;
    ...
    {
      foreach (Docflow.IOfficialDocument docAddenda in selectedAddenda)
      {
        if (docAddenda.LastVersion != null)
        {
          var lockInfoAddenda = Locks.GetLockInfo(docAddenda.LastVersion.PublicBody);
          if (lockInfoAddenda != null && lockInfoAddenda.IsLockedByMe)
          {
            Locks.Unlock(docAddenda.LastVersion.PublicBody);
          }
        }
      }
    }
                                    ...
                                    if (noSign)
                                    {
                                      ...
                                      if (signDialog.Show() == signButtons)
                                      {
                                        try
                                        {
                                          ...
                                          foreach (Docflow.IOfficialDocument docAddenda in selectedAddenda.Value)
                                          {
                                            if (docAddenda.LastVersion != null)
                                            {
                                              var bodyAddenda = docAddenda.LastVersion.PublicBody;
                                              var lockInfoAddenda = Locks.GetLockInfo(bodyAddenda);
                                              if (lockInfoAddenda != null)
                                              {
                                                if (lockInfoAddenda.IsLockedByOther)
                                                {
                                                  x.AddError(lockInfoAddenda.LockedMessage);
                                                  return;
                                                }
                                                else if (!lockInfoAddenda.IsLocked)
                                                {
                                                  Locks.TryLock(bodyAddenda);
                                                  lockedDocuments.Add(docAddenda);
                                                }
                                              }
                                            }
                                          }
